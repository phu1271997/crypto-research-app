...
from src.telegram_bot.formatters import format_error, format_platform_choice, format_topics_message
...
def _summarize_payload(payload: dict[str, Any] | None) -> str:
    ...
    if "topic" in payload and isinstance(payload["topic"], dict):
        ...
        return f"Topic hiện tại: {title} (platform: {platform})"
    ...
    if "article_meta" in payload and isinstance(payload["article_meta"], dict):
        ...
        if "image_paths" in payload and isinstance(payload["image_paths"], dict):
            ...
            summary = (
                ...
                f"thread={thread_count} tweet"
            )
            if isinstance(publish_result, dict):
                summary += f" | publish={', '.join(sorted(publish_result.keys()))}"
            return summary
        return f"Bài gần nhất: {article_title} ({word_count} từ) | platform={platform}"
...
async def _start_pipeline(
    ...
    await pipeline.run_full_research(update, context, topic, target_platform=target_platform)
...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    text = (
        ...
        "Các lệnh hiện có:\n"
        "/trending - Lấy 7 chủ đề hot ngay\n"
        ...
        "📢 Flow mới: Chọn topic → Chọn platform (Primus Spark hoặc AZDAG) → Bot viết bài riêng cho platform đó."
    )
...
async def cmd_trending(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("🔍 Đang lấy 7 chủ đề hot 48h qua...")
...
async def cmd_write(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # For /write, show platform choice
    user_id = _get_state_owner_id(update)
    await set_state(user_id, State.TOPICS_SHOWN, {"topics": [topic], "selected_topic": topic})
    text, keyboard = format_platform_choice(topic_text)
...
# ---------------------------------------------------------------------------
# Callback: Topic Selection → Platform Choice
# ---------------------------------------------------------------------------

async def callback_topic_select(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """User chọn topic → show platform choice buttons."""
    ...
    # Save selected topic into state, then ask for platform
    ...
    text, keyboard = format_platform_choice(title)
...
# ---------------------------------------------------------------------------
# Callback: Platform Selection → Start Pipeline
# ---------------------------------------------------------------------------

async def callback_platform_select(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """User chọn platform (primus/azdag) → bắt đầu pipeline."""
    if not _check_authorized(update) or update.callback_query is None:
        return
    ...
    target_platform = query.data.split(":", 1)[1]  # "primus" or "azdag"
    await query.answer(f"✅ Đăng bài cho {target_platform.upper()}.")

    platform_label = "🔵 Primus Spark (Web + X)" if target_platform == "primus" else "🟠 AZDAG (Web + X)"
    ...
    await _start_pipeline(update, context, selected_topic, target_platform=target_platform)
    ...
    keyboard = InlineKeyboardMarkup(
        [
            ...
            [
                ...
                InlineKeyboardButton("🐦 Chỉ thread", callback_data="regen:thread"),
            ],
...
async def callback_regen_action(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if action == "article":
        await context.bot.send_message(
            user_id,
            "ℹ️ Regen article sẽ tự động regen lại images + thread theo bài mới.",
        )
        await pipeline.regenerate_all(update, context)
        return

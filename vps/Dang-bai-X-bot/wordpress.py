...
class WordPressPublisher:
    ...
    async def delete_post(self, post_id: int | str, force: bool = True) -> dict:
        """
        Xóa bài WordPress theo post_id.
        """
        async for attempt in AsyncRetrying(
            stop=stop_after_attempt(3),
            wait=wait_exponential(multiplier=2, min=2, max=8),
            ...
            with attempt:
                ...
                deleted = bool(data.get("deleted")) or data.get("status") in {"trash", "deleted"}
                logger.info("✅ Xóa bài WP thành công: id={}, deleted={}", post_id, deleted)
                return {
                    "id": previous.get("id", post_id),
                    "link": previous.get("link") or data.get("link"),
                    "deleted": deleted,
                }

        raise RuntimeError("Xóa bài WP thất bại sau retry.")
...
async def publish_to_primus(
    ...
    try:
        ...
        html_content = _markdown_to_html_with_inline_image(article["content"], inline_url)
...
async def delete_from_primus(post_id: int | str | None, url: str | None = None) -> dict:
    """
    Main entry: xóa bài đã đăng trên Primus.
    """
    timestamp = datetime.now(UTC).isoformat().replace("+00:00", "Z")
    if post_id is None:
        return {
            ...
            "url": url,
            "post_id": None,
            "error": "Không có post_id để xóa bài Primus.",
            "attempted_at": timestamp,
        }

    publisher = WordPressPublisher()
    try:
        result = await publisher.delete_post(post_id, force=True)
        return {
            "status": "success" if result.get("deleted") else "failed",
            "url": result.get("link") or url,
            "post_id": result.get("id", post_id),
            "error": None if result.get("deleted") else "WordPress không xác nhận xóa bài thành công.",
            "attempted_at": timestamp,
        }
    except Exception as exc:  # noqa: BLE001
        logger.exception("❌ Xóa bài Primus thất bại")
        return {
            "status": "failed",
            "url": url,
            "post_id": post_id,
...
def mock_delete_from_primus(post_id: int | str | None, url: str | None = None) -> dict:
    timestamp = datetime.now(UTC).isoformat().replace("+00:00", "Z")
    return {
        "status": "success",
        "url": url or "https://primusspark.com/2026/05/MOCK-published-post/",
        "post_id": post_id,
        "error": None,
        "attempted_at": timestamp,
    }
